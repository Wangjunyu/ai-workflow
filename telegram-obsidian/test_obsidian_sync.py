...
class ObsidianSyncTests(unittest.TestCase):
    ...
    def test_append_can_ensure_extra_sections(self):
        with patch.object(self.sync, "_run_git"):
            with patch.object(self.sync, "_has_staged_changes", return_value=False):
                result = self.sync.append_to_inbox(
                    "hello sectioned world",
                    extra_section_headers=["## 今日概览", "## 业务推动"],
                )

        content = result["full_content"]
        self.assertIn("## 今日概览", content)
        self.assertIn("## 业务推动", content)
        self.assertIn("hello sectioned world", content)

    def test_upsert_section_adds_new_section(self):
        with patch.object(self.sync, "_run_git"):
            with patch.object(self.sync, "_has_staged_changes", return_value=False):
                result = self.sync.upsert_section("## 今日概览", "- 今日主线：测试")

        content = result["full_content"]
        self.assertIn("## 今日概览", content)
        self.assertIn("- 今日主线：测试", content)

    def test_upsert_section_replaces_existing_body(self):
        path = self.repo / "Journal/2026-06-05.md"
        path.write_text(
            "# 2026-06-05 周五\n\n## 今日概览\n\n- old body\n\n## NEXT\n\nbody\n",
            encoding="utf-8",
        ...
        with patch.object(self.sync, "_run_git"):
            with patch.object(self.sync, "_has_staged_changes", return_value=False):
                result = self.sync._upsert_section_locked(
                    path,
                    "Journal/2026-06-05.md",
                    "## 今日概览",
                    "- new body",
                    self.sync.tz_now().date(),
                )
